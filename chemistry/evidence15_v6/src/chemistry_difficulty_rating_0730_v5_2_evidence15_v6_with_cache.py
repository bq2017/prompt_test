...
MAX_SCHEMA_RETRIES = int(os.getenv("CHEMISTRY_CORE12_SCHEMA_RETRIES", "2"))
MAX_JSON_PARSE_RETRIES = int(os.getenv("CHEMISTRY_CORE12_JSON_RETRIES", "2"))
CORE12_POSTPROCESS_PROFILE = os.getenv(
    "CHEMISTRY_CORE12_POSTPROCESS_PROFILE", "core12_schema_only"
).strip()
        ...
        while True:
            try:
                ...
                try:
                    ...
                    repair_feedback = f"上次输出未通过Core-12 schema：{schema_error}"
                    continue
...
PROMPT_FILENAME = "0730初中化学难度打标提示词_v5_2_evidence15_v6.txt"
...
#   3. targeted_rules_v3只读取严格Core-12枚举，最多调整一个相邻档；
#   4. 不读取题目ID、教师标签或离线错题清单。
from chemistry_core12_schema import (  # noqa: E402
...
ALLOW_LEGACY_CORE12_SCHEMA = os.getenv(
    "CHEMISTRY_CORE12_ALLOW_LEGACY_SCHEMA",
    "0",
).strip().lower() in {"1", "true", "yes", "on"}
...
def postprocess_chemistry_difficulty(
    ...
    schema_only只校验结构；evidence15_boundary_rules_v6按15维证据交集
    ...
    if CORE12_POSTPROCESS_PROFILE not in {
        ...
        raise Core12SchemaError(
            f"未知Core-12后处理profile: {CORE12_POSTPROCESS_PROFILE!r}"
        )
    ...
    if CORE12_POSTPROCESS_PROFILE in {"core12_dataaware_rules_v5", "evidence15_boundary_rules_v6"}:
...
if __name__ == "__main__":
    print("0730 V5.2 Evidence-15版: images first + boundary evidence contract")
    print(f"0730 V5.2 Evidence-15后处理配置: {CORE12_POSTPROCESS_PROFILE}（定向规则需显式启用）")
    print(f"0724 V5.2 Core-12旧22维兼容: {'enabled' if ALLOW_LEGACY_CORE12_SCHEMA else 'disabled'}")
    print(f"0724 V5.2 image-primary图像输入: {'enabled' if ENABLE_IMAGE_INPUT else 'disabled'}")
