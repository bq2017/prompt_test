...
TAG="${3:-evidence15_v9_stage5_compact_591_$(date +%Y%m%d_%H%M%S)}"
...
export CHEMISTRY_EVIDENCE15_V9_POSTPROCESS_PROFILE="evidence15_boundary_rules_v9_stage5_audit45"
export CHEMISTRY_EVIDENCE15_V9_RUN_STAGE="stage5_compact"
...

{
  ...
  PYTHONPATH="${ROOT}/src${PYTHONPATH:+:${PYTHONPATH}}" \
    "${PYTHON[@]}" -m unittest -v \
      ...
      tests.test_evidence15_v9_stage5 \
      tests.test_evidence15_v9_stage5_compact

  echo "标准标签来源: 教师清洗CSV ${LABELS} 的 standard_level"
  echo "重要: 结果JSONL顶层 difficulty 是旧输入错误标签，正式评测一律忽略"
  echo "Stage5 Compact说明: 只压缩重复文本，4↔5逻辑和自动4→5仅审计策略不变"

  "${PYTHON[@]}" "${ROOT}/src/chemistry_difficulty_rating_evidence15_v9_with_cache.py" \
    --prompt "${ROOT}/prompts/evidence15_v9_stage5_compact_prompt.txt" \
    --input "${INPUT}" \
    --output "${RESULT}" \
    --error "${ERRORS}" \
    --concurrency 30
...
echo "V9 Stage5 Compact打标完成：${OUT_DIR}"
echo "结果包：${ARCHIVE}"
