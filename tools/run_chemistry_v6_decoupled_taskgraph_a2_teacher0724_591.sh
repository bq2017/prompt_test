...
TAG="${TAG:-0729_v6_decoupled_taskgraph_a2_teacher0724_591_run1}"
...
TASK_PROMPT="$PROJECT_ROOT/prompts/0730初中化学任务重建提示词_v6_decoupled_taskgraph_a2.txt"
RATING_PROMPT="$PROJECT_ROOT/prompts/0730初中化学难度打标提示词_v6_decoupled_taskgraph_a2.txt"
RUNNER="$PROJECT_ROOT/src/chemistry_difficulty_rating_0730_v6_decoupled_taskgraph_a2_with_cache.py"
...
for required in "$INPUT" "$LABELS" "$TASK_PROMPT" "$RATING_PROMPT" "$RUNNER" "$SCHEMA" "$EVALUATOR"; do
  if [[ ! -f "$required" ]]; then
    printf '缺失必要文件: %s\n' "$required" >&2
    exit 1
  fi
...
"${UV_PYTHON[@]}" -m py_compile "$RUNNER" "$SCHEMA" "$EVALUATOR" || exit 1
