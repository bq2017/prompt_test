...
TAG="${3:-evidence15_v9_stage4_591_$(date +%Y%m%d_%H%M%S)}"
...
export CHEMISTRY_EVIDENCE15_V9_RUN_STAGE="stage4"
...

{
  ...
  "${PYTHON[@]}" "${ROOT}/src/chemistry_difficulty_rating_evidence15_v9_with_cache.py" \
    --prompt "${ROOT}/prompts/evidence15_v9_stage4_prompt.txt" \
    --input "${INPUT}" \
    --output "${RESULT}" \
    --error "${ERRORS}" \
    --concurrency 30
