...
to imply a multi-question dependency.
"""
...
def validate_features(raw: Any) -> dict[str, str]:
    ...
    operation while ``reasoning_depth`` describes the complete continuous
    process, and internal dependency can exist in a single-question task.
    """
    return _require_exact_enum_object(raw, CORE_FEATURE_VALUES, "features")
...
def apply_safe_adjacent_level_change(
    ...
    """Apply one already-approved Evidence-15 rule while enforcing safety limits.
    ...
    result["postprocess_profile"] = "evidence15_v9_safe_rules"
...
def _evidence15_signal_counts(features: Mapping[str, str]) -> dict[str, int]:
    medium = sum([
...
def _apply_data_aware_change(
    ...
    result["coarse_difficulty"] = PAIR_COARSE[frozenset({old_level, new_level})]
    result["automatic_level_change_applied"] = True
    result["postprocess_profile"] = "evidence15_boundary_rules_v9_stage1"
    action = {
        "from": old_level,
        "to": new_level,
        "rule": rule,
...
def apply_data_aware_boundary_rules(
    ...
    counts = _evidence15_signal_counts(features)
    ...
    if level == "送分题":
        ...
        if semantic and (
            ...
            change = (
                ...
                "题面存在明确的规律应用动作，且15项审计特征至少记录一项实质转换，不能停在直接识别档。",
                structural, semantic,
            )
        ...
        if change is None and (experiment_chain or calculation_chain or shared_chain or entry in {"形成中间结论后应用", "多节点连续决策"}):
            ...
            change = (
                ...
                "题面与15项审计特征共同证明存在完整常规模型或连续依赖，超过一步应用。",
...
def _append_reasoning_calibration(
    ...
    *,
    ...
    reasoning["core_basis"] = (
        ...
        f"后处理仅做Evidence-15证据一致性校准：{reason}"
    ).strip()
    adjacent_text = {
        ("基础题", "送分题"): (
        ...
        ("中等题", "拔高题"): (
            "至少两个Evidence-15高阶维度共同指向会改变后续路径的卡点。",
...
def apply_targeted_evidence_rules(prepared: Mapping[str, Any]) -> dict[str, Any]:
    ...
    Evidence-15 enums.  They never inspect teacher labels, question IDs, keywords
    ...
    if not change:
        result["postprocess_profile"] = "evidence15_v9_targeted_rules"
        result["automatic_level_change_applied"] = False
