# -*- coding: utf-8 -*-
"""
@File    : chemistry_difficulty_rating_0730_v6_decoupled_taskgraph_a1_with_cache.py
@Description:
    V6决策隔离架构消融A1。复用V6稳定的缓存、图片输入、并发、重试和审计运输层，
    但替换生产输出合同：模型先按教师主标准独立定档，再输出最小任务图；
    程序只计算确定性的图指标，不再让Evidence-15同时承担定档和后处理触发。
    图片与完整结构化文字并行输入，降低任务边界重建波动。
"""
...
MAX_SCHEMA_RETRIES = int(os.getenv("CHEMISTRY_DECOUPLED_SCHEMA_RETRIES", "2"))
MAX_JSON_PARSE_RETRIES = int(os.getenv("CHEMISTRY_DECOUPLED_JSON_RETRIES", "2"))
DECOUPLED_POSTPROCESS_PROFILE = os.getenv(
    "CHEMISTRY_DECOUPLED_POSTPROCESS_PROFILE", "decoupled_schema_only"
).strip()
...
def build_user_content(data: Dict[str, Any], use_images: bool) -> Any:
    """构造图片与完整结构化文字并行的稳定题目输入。"""
    dynamic_text = (
        "这是图片—文字并行校验实验。请同时使用题干图片和结构化文字恢复完整题面；"
        "图片负责公式、装置、流程、曲线和空间关系，文字负责稳定题干、选项、解析和小问边界。"
        "二者冲突时按提示词的信息来源规则记录冲突，不得静默选择。"
    ...
    grounded_text = construct_question_content(data)
    if grounded_text:
        content.append(
            {
                "type": "input_text",
...
async def call_model_with_cache(
    ...
    grounded_text = construct_question_content(data)
    supplement_fields = [
        key for key in ("stem", "options", "analysis", "sub_questions")
        if data.get(key)
    ]
    image_status = {
        "question_input_mode": "image_text_grounded",
        "question_text_input_used": bool(grounded_text),
        "structural_metadata_used": True,
        "text_supplement_fields": supplement_fields,
        "text_supplement_char_count": len(grounded_text),
    ...
    async with semaphore:
        ...
        while True:
            try:
                ...
                try:
                ...
                except UnrateableInputError as exc:
                    raise RuntimeError(f"输入不可可靠定档，已拒绝猜测: {exc}") from exc
                    ...
                    repair_feedback = f"上次输出未通过决策隔离任务图schema：{schema_error}"
                    continue
                ...
                graph_metrics = rating_result.get("derived_features") or {}
                output_data["input_structure_audit"] = {
                    ...
                    "image_structure_inference_required": False,
                    "image_input_used": bool(image_status.get("image_input_used")),
                    "task_graph_node_count": graph_metrics.get("node_count", 0),
                    "task_graph_edge_count": graph_metrics.get("dependency_edge_count", 0),
                    "task_graph_cross_subquestion_edge_count": graph_metrics.get(
                        "cross_subquestion_edge_count", 0
                    ),
                    "blocking": False,
                    "note": (
                        "结构化小问未拆分；模型同时依据完整题干文字和图片建立任务图。"
                        if not structured_subquestions
                        else "结构化小问已提供；任务图用于审计真实跨问依赖。"
                    ),
                }
...
# -------------------------- 8. 0724 V5.2 image-primary/mintext独立后处理 --------------------------
# 为隔离输入模态，默认prompt_only；旧语义规则只保留用于非默认复现。
SCRIPT_DIR = Path(__file__).resolve().parent
PROJECT_ROOT = SCRIPT_DIR.parent
PROMPT_FILENAME = "0730初中化学难度打标提示词_v6_decoupled_taskgraph_a1.txt"
...
DEFAULT_CACHE = PROJECT_ROOT / "chemistry_0724_v5_2_image_primary_mintext_prompt_cache.json"
...
# -------------------------- 9. 决策隔离任务图生产契约 --------------------------
#
# 本版保留V6的传输层，只替换生产JSON和后处理入口：
#   1. 模型先按教师主标准独立输出difficulty_level；
#   2. task_graph记录实际任务节点和依赖边；
#   3. 程序只计算确定性的图指标；
#   4. 首轮实验不自动改档，只观察档位—任务图张力。
from chemistry_decoupled_taskgraph_schema import (  # noqa: E402
    DecoupledSchemaError as Core12SchemaError,
...
def postprocess_chemistry_difficulty(
    ...
    """唯一生产后处理入口；首轮实验不允许自动改档。"""
    if DECOUPLED_POSTPROCESS_PROFILE not in {
        "decoupled_schema_only",
        "decoupled_observe_only_v1",
    }:
        raise Core12SchemaError(
            f"未知决策隔离后处理profile: {DECOUPLED_POSTPROCESS_PROFILE!r}"
        )
    ...
    if DECOUPLED_POSTPROCESS_PROFILE == "decoupled_observe_only_v1":
        return apply_observe_only_audit(prepared)
    prepared["postprocess_profile"] = "decoupled_schema_only"
    prepared["automatic_level_change_applied"] = False
    return prepared

if __name__ == "__main__":
    print("0730 V6架构消融A1: independent rating + auditable task graph")
    print(f"0730 V6架构消融A1后处理配置: {DECOUPLED_POSTPROCESS_PROFILE}（首轮不自动改档）")
    print("0730 V6架构消融A1输入配置: full structured text + image grounding")
    print(f"0730 V6架构消融A1图像输入: {'enabled' if ENABLE_IMAGE_INPUT else 'disabled'}")
